...
- Ознайомилася з поняттям веб-доступності (accessibility) та її важливістю для створення інтерфейсів, якими можуть користуватися всі, включно з людьми з інвалідністю.

- Налаштувала ESLint у проєкті з використанням eslint-config-next/core-web-vitals, який включає плагін eslint-plugin-jsx-a11y для виявлення проблем доступності.

- Навчилася знаходити та виправляти базові помилки доступності, зокрема відсутність alt у зображеннях та некоректне використання aria-атрибутів.

- Покращила доступність форм за допомогою семантичного HTML, коректних <label> та візуального фокусу для навігації з клавіатури.

- Реалізувала валідацію форм на стороні сервера з використанням Server Actions та бібліотеки Zod.

- Ознайомилася з хуком useActionState для керування станом форми та обробки помилок після сабміту.

- Налаштувала відображення помилок для кожного поля форми з використанням aria-атрибутів (aria-describedby, aria-live, aria-atomic) для підтримки screen readers.

- Реалізувала загальні повідомлення про помилки форми та логіку безпечної взаємодії між клієнтом і сервером.

- Розширила серверну валідацію для форми редагування рахунку (edit form), зберігаючи єдиний підхід до обробки помилок.

![Скріншот структури дашборду](screenshots/chapter-13.png)

![Скріншот структури дашборду](screenshots/chapter-13(1).png)


**Роздуми:**

Цей розділ показав мені, що доступність — це не «додаткова опція», а невід’ємна частина якісного веб-додатка. Я зрозуміла, що навіть прості речі, як правильні <label>, alt-тексти або aria-атрибути, можуть суттєво вплинути на досвід користувачів.

Особливо цінним було поєднання server-side validation та useActionState. Такий підхід дозволяє мати одне надійне джерело істини для перевірки даних і водночас показувати користувачеві зрозумілі та доступні повідомлення про помилки.

Також я усвідомила, що доступність тісно пов’язана з UX: чіткі помилки, правильний фокус і логічна структура форми роблять застосунок зручнішим для всіх, а не лише для користувачів з особливими потребами.
...
- Ознайомилася з фундаментальною різницею між автентифікацією (підтвердження особи користувача) та авторизацією (визначення прав доступу до ресурсів).

- Налаштувала бібліотеку NextAuth.js (версія beta для Next.js 14+) для керування сесіями та процесом входу/виходу.

- Реалізувала захист конфіденційних даних за допомогою секретного ключа AUTH_SECRET, згенерованого для шифрування файлів cookie.

- Створила кастомну сторінку входу /login, інтегрувавши її в налаштування NextAuth за допомогою опції pages.

- Налаштувала Middleware (через файл proxy.ts) для автоматичного захисту маршрутів дашборду. Тепер неавторизовані користувачі автоматично перенаправляються на сторінку входу.

- Реалізувала логіку перевірки облікових даних користувача через CredentialsProvider, використовуючи базу даних Postgres.

- Забезпечила безпеку паролів, використовуючи бібліотеку bcrypt для порівняння введеного пароля з його хешованим значенням у базі даних.

- Підключила логіку автентифікації до клієнтської форми входу за допомогою Server Action та хука useActionState, забезпечивши обробку помилок (наприклад, "Invalid credentials").

- Додала функціонал виходу із системи (signOut) у боковій панелі навігації.

Ввела логін і пароль
![Скріншот структури дашборду](screenshots/chapter-14(login).png)

Після входу доcтупні всі сторінки і з'явилась кнопка sign out
![Скріншот структури дашборду](screenshots/chapter-14(afterlogin).png)

Спроба зайти під неіснуючим user
![Скріншот структури дашборду](screenshots/chapter-14(incorrectlogin).png)

**Роздуми:**

Цей розділ допоміг мені зрозуміти, як будується безпека в сучасних веб-додатках. Найбільшим відкриттям стало використання Middleware — це дуже потужний інструмент, який дозволяє захистити цілі розділи сайту всього кількома рядками коду, не прописуючи перевірки на кожній сторінці окремо.

Також я усвідомила важливість безпечного зберігання паролів. Використання bcrypt для хешування — це стандарт, який захищає користувачів навіть у разі витоку даних із бази.

Робота з NextAuth спочатку здавалася заплутаною через велику кількість файлів (auth.config.ts, auth.ts, proxy.ts), але тепер я бачу логіку: кожен файл відповідає за свій етап (налаштування, стратегія перевірки та перехоплення запитів). Це робить код структурованим і легким для масштабування.
...
- Ознайомилася з поняттям метаданих та їхньою важливою роллю в покращенні SEO (пошукової оптимізації) та привабливості посилань у соціальних мережах.

- Вивчила основні типи метаданих, такі як заголовок (title), опис (description), ключові слова (keywords) та метадані Open Graph для відображення прев'ю в соцмережах.

- Налаштувала автоматичну ідентифікацію іконок сайту та зображень для соцмереж, перемістивши файли favicon.ico та opengraph-image.jpg у корінь папки /app.

- Налаштувала шаблон заголовків (title.template) у кореневому макеті, що дозволило автоматично додавати назву компанії до унікальних заголовків кожної сторінки.

- Застосувала знання на практиці, додавши індивідуальні заголовки для сторінок /login, /dashboard, /dashboard/customers, а також для сторінок створення та редагування інвойсів.

- Переконалася, що вкладені метадані коректно замінюють батьківські, забезпечуючи точну інформацію для кожної конкретної сторінки.

![Скріншот структури дашборду](screenshots/chapter-15(meta).png)

**Роздуми:**

Цей розділ допоміг мені зрозуміти, що якісний веб-застосунок має бути не лише функціональним, а й "видимим" для зовнішнього світу. Я дізналася, що метадані — це місток між моїм кодом та пошуковими роботами або алгоритмами соцмереж.

Особливо зручним виявилося використання шаблонів заголовків у Next.js. Це значно спрощує підтримку коду: я можу один раз прописати назву бренду в layout.tsx, а на окремих сторінках вказувати лише назву розділу. Це позбавляє від дублювання тексту і робить систему гнучкою до змін.

Також було цікаво дізнатися про файловий підхід до метаданих. Те, що Next.js автоматично генерує потрібні теги в <head> просто на основі наявності файлу favicon.ico або opengraph-image.jpg у папці /app, робить процес розробки набагато швидшим і приємнішим.
